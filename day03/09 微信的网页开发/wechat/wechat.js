...
const {writeData,readData}=require("../tools")
...
class Wechat {
     ...
     saveAccessToken(filePath,accessToken){
         return writeData(filePath,accessToken)
     }
     readAccessToken(filePath){
         return readData(filePath)
     }
     isInTimeAccessToken({expires_in}){
     ...
     async fetchAccessToken(){
         ...
         if(this.access_token&&this.expires_in&&this.isInTimeAccessToken(this)){
                console.log("进入if")
         ...
         return this.readAccessToken("./accessToken.txt")
                 .then(async res=>{
                     ...
                     if(this.isInTimeAccessToken(res)){
    ...
    async getTicket(){
      ...
      // 异步任务 获取accessToken 不可能一瞬间获取完 所以需要用async
      let {ticket,expires_in}=await rp({method:"GET",url,json:true}) // 获取accessToken并且转换成js对象
      // 设置accessToken的过期时间
      expires_in=Date.now()+ 7200000-1000*300
      return {ticket,ticket_expires_in:expires_in}
    }
    saveTicket(filePath,ticket){
      return writeData(filePath,ticket)
    }
    readTicket(filePath){
      return readData(filePath)
    }
    isInTimeTicket({ticket_expires_in}){
      // 为true的话 没有过期 为false的话 过期了
    ...
    async fetchTicket(){
      ...
      // err对象为true
      // 如果调用了access  如果有accessToken并且没有过期的话 直接使用
      if(this.ticket&&this.ticket_expires_in&&this.isInTimeTicket(this)){
        console.log("进入if")
        return {ticket:this.ticket,ticket_expires_in:this.ticket_expires_in}
      }
      return this.readTicket("./ticket.txt")
        ...
        .then(async (ticket)=>{
          ...
          this.ticket_expires_in=ticket.ticket_expires_in
          return ticket
        })
    }
